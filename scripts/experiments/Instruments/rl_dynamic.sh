...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/rl_outputs/Instruments-grec-genrec-dynamic-hint-from-sft}"
RUN_NAME="${RUN_NAME:-instruments_grec_genrec_dynamic_hint_from_sft}"
...
LAUNCH_ARGS=(
  ...
  --main-port "$MAIN_PORT"
  --grad-acc "$GRAD_ACC"
  --per-device-train-bsz "$PER_DEVICE_TRAIN_BSZ"
  --per-device-eval-bsz "$PER_DEVICE_EVAL_BSZ"
